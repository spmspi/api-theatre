...
from theatre.models import Play, TheatreHall, Performance, Genre, Actor, Reservation
from theatre.serializers import PlayListSerializer, PlayDetailSerializer

THEATRE_URL = reverse("theatre:play-list")
THEATRE__SESSION_URL = reverse("theatre:performance-list")
...
class CreatedReservationApiTests(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = get_user_model().objects.create_user(
            "test@test.com",
    ...
    def test_create_ticket(self):
        ...
        payload = {
            "row": 2,
            "seat": 1,
            "performance": performance_session.id,
